  ...
  return (
    <main className="container mx-auto px-4 py-12">
      <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-xl overflow-hidden">
        ...
        <div className="p-8">
          <h1 className="text-4xl font-bold mb-3 text-gray-800">{recipe.title}</h1>
          ...
          <p className="text-gray-700 mb-5">{recipe.description}</p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {relatedRecipes.map((rec) => (
            <Link href={`/recipe/${rec.slug}`} key={rec.id}>
              <div className="shadow-lg rounded-lg overflow-hidden hover:shadow-2xl transition duration-300">
                <div className="relative h-48">
                  <Image src={rec.image} alt={rec.title} fill style={{ objectFit: "cover" }} />
                </div>
                <div className="p-4">
                  <h3 className="font-semibold text-gray-800 truncate">{rec.title}</h3>
                </div>
              </div>
            </Link>
          ))}
